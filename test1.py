...
def chat_with_gpt(prompt):
    ...
    if ChatWithAIBaseUrl == "https://openrouter.ai/api/v1/chat/completions":
        json_model["plugins"] = [{"id": "web"}]
    try:
...
if __name__ == "__main__":
    ...
    # BaseUrl
    ChatWithAIBaseUrl = conf['BaseUrl']['ChatWithAIBaseUrl']
    DelSearchBaseUrl = conf['BaseUrl']['DelSearchBaseUrl']
    ZhiPu = conf['BaseUrl']['ZhiPu']
    Deepseek = conf['BaseUrl']['Deepseek']
    XzSpeakBaseUrl = conf['BaseUrl']['XzSpeakBaseUrl']
    # Model
    ChatWithAIModel = conf['Model']['ChatWithAIModel']
    DelSearchModel = conf['Model']['DelSearchModel']
    XzSpeakModel = conf['Model']['XzSpeakModel']
    sc = """你是专注于**语音识别文本修正**的工具，仅对用户提供的“语音识别文本”执行修正操作，严格遵守以下规则，禁止回应任何提问、解释自身功能或添加无关内容：
1. **错字/误词修正**：精准修正语音识别中的错别字、混淆词（例：用户输入“什么是变成”，输出“什么是编程？”）；
2. **标点补充**：按语义逻辑添加必要标点（逗号、句号、问号等），不堆砌冗余符号；
3. **重点词汇强校验**：优先检测“小宇同学”“退出”“再见”，修正语音误听（例：“额黑消雨同学”→“小宇同学”，“你嘿小宇同学”→“你好，小宇同学”）；
